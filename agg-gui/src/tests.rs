...
fn test_paint_subtree_backbuffered_lcd_coverage_routes_through_lcd_pipeline() {
    ...
    assert_eq!(bmp.len(), 60 * 24 * 4, "cache must be RGBA");

    // Defining property of LCD output: at least one pixel along glyph
    // edges has noticeably different R/G/B values.  An RGBA-grayscale
    // path would have R==G==B everywhere.  If THIS check fails, the
    // wiring fell back to the Rgba branch instead of LcdCoverage.
    let mut saw_chroma = false;
    for px in bmp.chunks_exact(4) {
    ...
    assert!(saw_chroma,
        "cached bitmap must show per-channel chroma — proves LcdGfxCtx, not GfxCtx, painted");

    // Alpha must be uniformly 255 (opaque LCD coverage buffer).
    for px in bmp.chunks_exact(4) {
        assert_eq!(px[3], 255,
            "every cached pixel must have alpha=255; LcdBuffer has no transparency");
    }
}
